...
Public Class Frm_DealSummary_Interface
    ...
    Private Sub Frm_DealSummary_Interface_KeyDown(ByVal sender As Object, ByVal e As System.Windows.Forms.KeyEventArgs) Handles Me.KeyDown
    ...
    Private Sub Frm_DealSummary_Interface_Load(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles MyBase.Load
        Me.KeyPreview = True
        conn = New CSQL
        conn.Connect()
    ...
    Private Sub FillCombo(Optional ByVal strControl As String = "")
        ...
        'Call conn.Fill_ComboBox(strSQL, cbo_checksum_algorithm)e

        If strControl <> "cbo_interface_type" Then
            cbo_checksum_algorithm.Items.Clear()
            cbo_checksum_algorithm.Items.Add("")
            cbo_checksum_algorithm.Items.Add("CRC32")
            cbo_checksum_algorithm.Items.Add("MD5")
            cbo_checksum_algorithm.Items.Add("ALR32")
        End If
    ...
    Private Sub ShowData(Optional ByVal Copydata As Boolean = False)
        Dim res As SqlDataReader
        Dim strSQL As String = ""

        If Copydata = False Then
            strSQL = "SELECT * FROM [TB_DEAL_INTERFACE] WHERE [company_id] ='" + txt_company_id.Text.Replace("'", "''") + "'  and mapping_code='" + txt_mapping_code.Text.Replace("'", "''") + "' "
        Else
            strSQL = "SELECT * FROM [TB_DEAL_INTERFACE_TEMPLATE] WHERE  mapping_code='" + txt_mapping_code.Text.Replace("'", "''") + "' "
            'txt_mapping_code.Text = ""
        End If
    ...
    Private Sub SaveData()
        Try
            ...
            If strMODE.ToUpper <> "ADD" Then
                strSQL = ""
                strSQL += vbCrLf & "update [TB_DEAL_INTERFACE] set "
                ...
                strSQL += vbCrLf & " WHERE [company_id] ='" + txt_company_id.Text.Replace("'", "''") + "'  and mapping_code='" + txt_mapping_code.Text.Replace("'", "''") + "' "
                conn.ExecuteNonQuery(strSQL)
                Me.Close()
    ...
    Private Sub btnFind_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles btnFind.Click
        Dim mFrm_DealSummary_Interface_Mnt_List As Frm_DealSummary_Interface_Mnt_List
        mFrm_DealSummary_Interface_Mnt_List = New Frm_DealSummary_Interface_Mnt_List

        mFrm_DealSummary_Interface_Mnt_List.is_query = True
        mFrm_DealSummary_Interface_Mnt_List.txtCodeReturn = txt_mapping_code

        mFrm_DealSummary_Interface_Mnt_List.btn_Select.Visible = True
        mFrm_DealSummary_Interface_Mnt_List.btn_Add.Visible = False
        mFrm_DealSummary_Interface_Mnt_List.btn_Delete.Visible = False
        mFrm_DealSummary_Interface_Mnt_List.btn_Edit.Visible = False
